...
pub static mut CLI_INPUT_QUEUE: Queue<u8, U512> = Queue(heapless::i::Queue::new());
/// The producer end of `CLI_INPUT_QUEUE`. Bytes are put into the queue in the interrupt handler `USB`
pub static mut CLI_INPUT_PRODUCER: Option<Producer<u8, U512>> = None;
...
pub const USB_INTERRUPTS: [interrupt; 3] = [
    interrupt::USB_OTHER,
    interrupt::USB_TRCPT0,
    interrupt::USB_TRCPT1,
];
...
#[must_use]
pub fn get_serial_input_pipe() -> Consumer<'static, u8, U512> {
